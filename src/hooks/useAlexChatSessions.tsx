...
export function useAlexChatSessions() {
  ...
  // Criar nova sessão
  const createSession = useCallback(async (): Promise<AlexChatSession | null> => {
    if (!user) return null;
    ...
    try {
      const { data, error } = await supabase
        ...
        .insert({
          user_id: user.id,
          title: 'Nova Conversa com Alex IA',
          auto_title: true,
        })
        .select()
        .single();
      ...
      const newSession = data as AlexChatSession;
  ...
  const sendMessage = useCallback(async (content: string): Promise<boolean> => {
    ...
    try {
      ...
      const tempUserMessage: AlexChatMessage = {
      ...
      // Chamar Edge Function da Alex IA
  ...
  const togglePin = useCallback(async (sessionId: string) => {
    ...
    try {
      ...
      if (currentSession?.id === sessionId) {
        setCurrentSession(prev => prev ? { ...prev, pinned: !session.pinned } : null);
      }
    } catch (error) {
  ...
  // Reset completo - apagar todas as conversas
  const resetAllSessions = useCallback(async () => {
    ...
    try {
      ...
      toast({
        title: "Erro",
        description: "Não foi possível resetar as conversas",
        variant: "destructive",
      });
    }
  }, [user, toast]);
