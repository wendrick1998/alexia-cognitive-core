...
import { LLMProvider, TaskType } from '@/hooks/useLLMRouter';
...
export class LLMLogger {
  ...
  async flushLogs(): Promise<void> {
    ...
    try {
      ...
      const formattedLogs = logsToFlush.map(log => ({
        ...
        // Remover propriedades originais de data que serão substituídas
        startTime: undefined,
        endTime: undefined
      }));
  ...
  async getFallbackMetrics(): Promise<{
    ...
    try {
      ...
      const { data: fallbackLogs, error: fallbackError } = await this.supabase
        .from('llm_call_logs')
        .select('*')
      ...
      const { data: nonFallbackLogs, error: nonFallbackError } = await this.supabase
        .from('llm_call_logs')
        .select('*')
